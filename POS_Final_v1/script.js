  ...
  if(isCash){
    ...
    paySection=`<div class="ewallet-wrap" id="ewalletWrap">
      <div class="ewallet-tabs">
        <button class="ewallet-tab active" id="tabPaymaya" onclick="switchEwallet('paymaya')">💜 PayMaya</button>
        <button class="ewallet-tab" id="tabGcash" onclick="switchEwallet('gcash')">💙 GCash</button>
      </div>
      <div id="qrArea">
        <div class="qr-brand" id="ewalletBrand">PayMaya</div>
        <div class="qr-box" id="qrBox"><div id="qrSvg">${generateQR("PAYMAYA_"+total.toFixed(2))}</div><div class="qr-scan-line"></div></div>
        <div class="qr-amount">${php(total)}</div>
        <div class="qr-inst">Open your <span id="ewalletApp">PayMaya</span> app and scan the QR code</div>
...
function finaliseOrder(payment,tendered,total,customer,orderType,persons,sub,tax,_total){
  ...
  // Deduct stock (only for beans/pastries)
  ...
  const receiptHTML=`
    <div class="receipt-paper" id="receiptContent">
      ...
      <hr>
      <div style="font-size:11px;">Order #${order.id}</div>
  ...
  modalFooter.innerHTML = `<button class="btn-confirm" id="receiptActionBtn" style="width:100%;">🖨️ Print Receipt</button>`;

  // Hide scan button area
  const simBtn=$("#simScanBtn");if(simBtn)simBtn.style.display="none";

  const actionBtn = $("#receiptActionBtn");
  actionBtn.addEventListener("click", function handleReceiptAction() {
    // If button is already in "Back" mode, just close the modal.
    ...
    // 3. Transform the button into a "Back" button
    this.textContent = "⬅️ Back";
    this.dataset.action = 'back';
    this.classList.replace('btn-confirm', 'btn-outline');
  });
}
